...
import {
  ...
  Platform, RefreshControl, Modal, ActivityIndicator, Image, Alert, KeyboardAvoidingView
} from "react-native";
...
export default function ProgresoTrainerScreen() {
  ...
  const [brazo, setBrazo] = useState(""); // Solo hombres
  const [cadera, setCadera] = useState(""); // Solo mujeres
  const [muslo, setMuslo] = useState(""); // Solo mujeres
  ...
  const createMutation = useMutation({
    mutationFn: async () => {
      ...
      // Limpiamos los campos que no correspondan al género antes de enviar a Supabase
      const isHombre = genero === "hombre";
      const brazoFinal = isHombre && brazo ? Number(brazo) : null;
      const caderaFinal = !isHombre && cadera ? Number(cadera) : null;
      const musloFinal = !isHombre && muslo ? Number(muslo) : null;
        ...
        {latestEntry ? (
          <View style={styles.metricsCard}>
            <View style={styles.metricsHeaderRow}>
              ...
              {latestEntry.genero && (
                <View style={styles.genderBadge}>
                  <Ionicons name={latestEntry.genero === "hombre" ? "man" : "woman"} size={14} color={Colors.primary} />
            ...
            <View style={styles.metricsGrid}>
              ...
              {/* Mostramos brazo SOLO si es hombre o si tiene valor en DB */}
              {(latestEntry.genero === "hombre" || latestEntry.brazo) && latestEntry.brazo && (
                <View style={styles.metricItem}>
                  <Text style={styles.metricValue}>{latestEntry.brazo}</Text>
                  <Text style={styles.metricLabel}>cm brazo</Text>
                </View>
              )}
              {/* Mostramos cadera/muslo SOLO si es mujer o si tienen valor en DB */}
              {(latestEntry.genero === "mujer" || latestEntry.cadera) && latestEntry.cadera && (
                <View style={styles.metricItem}>
                  <Text style={styles.metricValue}>{latestEntry.cadera}</Text>
                  <Text style={styles.metricLabel}>cm cadera</Text>
                </View>
              )}
              {(latestEntry.genero === "mujer" || latestEntry.muslo) && latestEntry.muslo && (
                <View style={styles.metricItem}>
          ...
          entries.map((entry: any) => (
            <View key={entry.id} style={styles.entryCard}>
              ...
              <View style={styles.entryMetrics}>
                ...
                {(entry.genero === "hombre" || entry.brazo) && entry.brazo && (
                  <Text style={styles.entryMetric}>{entry.brazo} cm brazo</Text>
                )}
                {(entry.genero === "mujer" || entry.cadera) && entry.cadera && (
                  <Text style={styles.entryMetric}>{entry.cadera} cm cadera</Text>
                )}
                {(entry.genero === "mujer" || entry.muslo) && entry.muslo && (
                  <Text style={styles.entryMetric}>{entry.muslo} cm muslo</Text>
                )}
              </View>
              {entry.notas && (
        ...
        <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === "ios" ? "padding" : undefined}>
          ...
          <View style={[styles.modal, { paddingBottom: insets.bottom + 24 }]}>
            ...
            >
              ...
              <Text style={[styles.sectionTitle, { fontSize: 16, marginTop: 4, marginBottom: 12 }]}>Circunferencias (cm)</Text>
              
              {/* CAMBIAMOS LA VISTA SEGÚN EL GÉNERO */}
              {genero === "hombre" ? (
                <View style={styles.row}>
                  <View style={{ flex: 1 }}>
